...
<div class="row-fluid login-wrapper">
    ...
    <div class="span4 box">
        ...
        <div class="content-wrap">
            <h6>慕课商城 - 修改密码</h6>
            <?php if (Yii::$app->session->hasFlash('update_success')) {
                echo Yii::$app->session->getFlash('update_success');
            } ?>
            <?= $form->field($model, 'adminpass')->passwordInput(['class' => 'span12' , 'placeholder' => '请输入新密码']) ?>
            <?= $form->field($model,'pass')->passwordInput(['class' => 'span12' , 'placeholder' => '请再次输入密码'])?>
            <a href="?r=login/index" class="forgot">返回登录</a>
            <?= Html::submitButton('修改密码', ['class' => 'btn-glow primary login']) ?>
        </div>
        <?php ActiveForm::end();?>
    </div>
